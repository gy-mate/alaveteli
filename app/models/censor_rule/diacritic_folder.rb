class CensorRule::DiacriticFolder
  # Maps each character to a regexp character class covering its diacritic
  # variants. Separate lowercase and uppercase entries preserve the original
  # case of the input — when case_sensitive is false Regexp::IGNORECASE
  # covers the other case without enumerating both in each class.
  #
  # Reusers can replace or extend this for their locale:
  #   CensorRule::DiacriticFolder.character_map = { ... }
  ...
  def fold(text, encoding: 'UTF-8')
    text.mb_chars.each_char.map { fold_character(it) }.join.encode(encoding)
  end
