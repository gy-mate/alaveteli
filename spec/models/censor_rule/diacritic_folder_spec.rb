...
RSpec.describe CensorRule::DiacriticFolder do
  ...
  describe '#fold' do
    context 'with default case-sensitive mode' do
      ...
      it 'folds lowercase characters with diacritics to character classes' do
        expect(folder.fold('a')).to eq('[aàáâãäåā]')
      end

      it 'folds uppercase characters with diacritics to character classes' do
        expect(folder.fold('A')).to eq('[AÀÁÂÃÄÅĀ]')
      end

      it 'preserves characters without diacritic variants' do
        expect(folder.fold('b')).to eq('b')
      end

      it 'ignores non-mapped characters' do
        expect(folder.fold('123')).to eq('123')
        expect(folder.fold('!@*')).to eq('!@*')
      end

      it 'handles multi-character strings' do
        expect(folder.fold('café')).to eq('[cçćč][aàáâãäåā]f[eèéêëēě]')
      end

      it 'handles mixed mapped and non-mapped characters' do
        expect(folder.fold('a1b2c')).to eq('[aàáâãäåā]1b2[cçćč]')
      end
      ...
      it 'handles strings with spaces and punctuation' do
        ...
        expect(folder.fold('café au lait')).to eq(expected)
      end
    ...
    context 'with case-insensitive mode' do
      subject(:folder) { described_class.new(case_sensitive: false) }

      it 'merges character classes for both cases' do
        expect(folder.fold('a')).to eq('[aàáâãäåāAÀÁÂÃÄÅĀ]')
        expect(folder.fold('A')).to eq('[aàáâãäåāAÀÁÂÃÄÅĀ]')
      end

      it 'handles mixed case strings' do
        expect(folder.fold('Café')).
      ...
      it 'handles diacritic characters in case-insensitive mode' do
        expect(folder.fold('á')).to eq('[aàáâãäåāAÀÁÂÃÄÅĀ]')
        expect(folder.fold('Á')).to eq('[aàáâãäåāAÀÁÂÃÄÅĀ]')
      end
    end
  end
  ...
  describe '#case_sensitive?' do
    it 'returns true by default' do
      ...
      expect(folder.case_sensitive?).to eq(true)
    end

    it 'returns false when case_sensitive is set to false' do
      folder = described_class.new(case_sensitive: false)
      expect(folder.case_sensitive?).to eq(false)
    end
  end
end
